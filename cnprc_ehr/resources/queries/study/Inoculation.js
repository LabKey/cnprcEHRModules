...
function onUpsert(helper, scriptErrors, row, oldRow) {
    if (!helper.isETL()) {
        ...
        if (row.date) {
            ...
            if (date > hi_date) {
                EHR.Server.Utils.addError(scriptErrors, 'date', 'The Inoculation date is above the high Date for Animal, which is '+new Date(hi_date), 'ERROR');
            }
            else if (date < lo_date) {
                EHR.Server.Utils.addError(scriptErrors, 'date', 'The Inoculation date is below the low Date for Animal, which is '+new Date(lo_date), 'ERROR');
            }
        }
